...
static const int PLAYER_LOAD_TIMEOUT = 60;

static const NSString* KEY_TIMESTAMP = @"timestamp";
static const NSString* KEY_DURATION = @"duration";    //micro seconds
static const NSString* KEY_POSITION = @"video_pos";     //milli seconds
static const NSString* KEY_POSITION_START = @"video_start";
static const NSString* KEY_POSITION_END = @"video_end";
static const NSString* KEY_STATE_TYPE = @"type";
...
typedef enum{
    enumPlayerBegin = 0,
    enumPlayerBeginLoadBegin = 1,
    enumPlayerSeekLoadBegin = 2,
    enumPlayerSeekBegin = 5,
    enumPlayerPauseBegin = 6,
    enumPlayerExit = 7,
    enumPlayerNormalLoadBegin = 9,
    enumPlayerPlayBegin = 10,
}PlayerRunStatus;

typedef enum{
    enumPlayerBeginLoadDuration = 1,
    enumPlayerSeekLoadDuration = 2,
    enumPlayerNormalLoadDuration = 3,
    enumPlayerPlayDuration = 4,
    enumPlayerPauseDuration = 8,
}PlayerDurationStatus;
...
@implementation IJKFFMoviePlayerController {
    ...
    NSTimer *_checkLoadTimer;
    int _startTimePos;
    
    //caculate run state time log
...
@synthesize isReportLog = _isReportLog;
             ...
             withOptions:(IJKFFOptions *)options playView:(UIView *)playView
{
    ...
    return [self initWithContentURLString:aUrlString
                              withOptions:options playView:playView];
}


-(void)onCheckLoadTimeout{
    if (_loadState==IJKMPMovieLoadStateStalled) {
        int loadDuration = (int)(SDL_GetTickHR() - _monitor.lastPrerollStartTick)/1000;
        if (loadDuration>PLAYER_LOAD_TIMEOUT) {
            NSLog(@"player load timeout duration=%d, you need to restart", loadDuration);
            _loadState = IJKMPMovieLoadStateTimeout;
...
- (void)startCheckLoadTimer
{
    if (_checkLoadTimer != nil)
        return;
    
    if ([[NSThread currentThread] isMainThread]) {
        _checkLoadTimer = [NSTimer scheduledTimerWithTimeInterval:1.0f target:self selector:@selector(onCheckLoadTimeout) userInfo:nil repeats:YES];
    } else {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self startCheckLoadTimer];
        });
    }
}

- (void)stopCheckLoadTimer
{
    if (_checkLoadTimer == nil)
        return;
    
    if ([[NSThread currentThread] isMainThread]) {
        [_checkLoadTimer invalidate];
        _checkLoadTimer = nil;
    } else {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self stopCheckLoadTimer];
        });
    }
}
                   ...
                   withOptions:(IJKFFOptions *)options playView:(UIView *)playView
{
    _lastRunState = -1;
    _playBegin = NO;
    [self reportCurrentState: enumPlayerBegin];
    ...
    if (self) {
        ...
        // init video sink
        if (playView!=nil&&[playView isKindOfClass:[IJKSDLGLView class]]){
            _glView = (IJKSDLGLView*)playView;
        }else{
       ...
       // [_glView setHudValue:nil forKey:@"t-preroll"];
...
#ifdef DEBUG
        [self setLogLevel:k_IJK_LOG_DEBUG];
#else
        [self setLogLevel:k_IJK_LOG_SILENT];
#endif
        // init audio sink
        ...
        _networkInfo = [[STNetworkInfo alloc] init];
        [_networkInfo initNetworkInfo];
...
-(void)setFastDisplay:(BOOL)fastDisplay{
    if (!_mediaPlayer)
        return;
     _fastDisplay = fastDisplay;
    ijkmp_set_option_int(_mediaPlayer, IJKMP_OPT_CATEGORY_PLAYER, "force-refresh", fastDisplay ? 1 : 0);
}

-(void)setHttpKeepAlive:(BOOL)httpKeepAlive{
    if (!_mediaPlayer)
        return;
    _httpKeepAlive = httpKeepAlive;
    ijkmp_set_option_int(_mediaPlayer, IJKMP_OPT_CATEGORY_PLAYER, "hls-http-keepalive", httpKeepAlive? 1 : 0);
}

#pragma mark 上报播放状态数据

-(void)insertDurationStateLog: (int)state duration:(NSNumber*)stateDuration curTimestamp:(NSNumber*)curTimestampNumber pos:(NSNumber*) curPosition{
    NSMutableDictionary* dic = [NSMutableDictionary dictionary];
    NSNumber* curState = [NSNumber numberWithInt:state];
    [dic setObject:curState forKey:KEY_STATE_TYPE];
    [dic setObject:curTimestampNumber forKey: KEY_TIMESTAMP];
    [dic setObject:stateDuration forKey:KEY_DURATION];
    if (curPosition) {
        [dic setObject:curPosition forKey:KEY_POSITION];
    }
    [[STLogReporter instance] insertStateLog:dic];
}

-(void)updateStateTimestamp: (int)state timestamp:(NSNumber*)curTimestamp pos:(NSNumber*)curPosition{
    NSNumber* curState = [NSNumber numberWithInt:state];
    switch (state) {
        case enumPlayerNormalLoadBegin:
        case enumPlayerBegin:
        case enumPlayerPlayBegin:
        case enumPlayerPauseBegin: {
            NSMutableDictionary* dic = [NSMutableDictionary dictionary];
            [dic setObject:curState forKey:KEY_STATE_TYPE];
            [dic setObject:curTimestamp forKey: KEY_TIMESTAMP];
            [dic setObject:curPosition forKey:KEY_POSITION];
            [[STLogReporter instance] insertStateLog:dic];
        }
            break;
        case enumPlayerSeekBegin:{
            NSMutableDictionary* dic = [NSMutableDictionary dictionary];
            [dic setObject:curState forKey:KEY_STATE_TYPE];
            [dic setObject:curTimestamp forKey: KEY_TIMESTAMP];
            [dic setObject:curPosition forKey:KEY_POSITION_START];
            [dic setObject:[NSNumber numberWithInt:_seekTargetTime*1000] forKey:KEY_POSITION_END];
            [[STLogReporter instance] insertStateLog:dic];
        }
            break;
        case enumPlayerExit:{
    ...
    _lastTimestamp = av_gettime();
    _lastRunState = state;
    NSLog(@"update state to %d", _lastRunState);
    
}

-(void)reportCurrentState:(int)state{
    if ( _lastRunState==state) {
        //same state do nothing
        NSLog(@"current state is same to last, do nothing, state=%d", state);
        return;
    }
    int64_t curTimestamp = av_gettime();
    NSNumber* stateDuration = [NSNumber numberWithLongLong: (curTimestamp-_lastTimestamp)];
    NSNumber* curTimestampNumber = [NSNumber numberWithLongLong:curTimestamp/1000];
    NSNumber* curPosition = [NSNumber numberWithLong:(long)[self currentPlaybackTimeMilliSeconds]];
    NSLog(@"current play position = %ld", curPosition.longValue);
    //caculate all duration
    switch (state) {
        case enumPlayerPlayBegin:{
            //loading and pause duration
            if (_lastRunState==enumPlayerBeginLoadBegin) {
                [self insertDurationStateLog:enumPlayerBeginLoadDuration duration:stateDuration curTimestamp:curTimestampNumber pos:curPosition ];
            }else if(_lastRunState==enumPlayerSeekLoadBegin){
                [self insertDurationStateLog:enumPlayerSeekLoadDuration duration:stateDuration curTimestamp:curTimestampNumber pos:curPosition];
            }else if(_lastRunState==enumPlayerNormalLoadBegin){
                [self insertDurationStateLog:enumPlayerNormalLoadDuration duration:stateDuration curTimestamp:curTimestampNumber pos:curPosition];
            }else if( _lastRunState==enumPlayerPauseBegin){
                [self insertDurationStateLog:enumPlayerPauseDuration duration:stateDuration curTimestamp:curTimestampNumber pos:nil];
            }
        }
        case enumPlayerNormalLoadBegin:
            //played duration
            if (_lastRunState==enumPlayerPlayBegin) {
                [self insertDurationStateLog:enumPlayerPlayDuration duration:stateDuration curTimestamp:curTimestampNumber pos:nil];
            }
            break;
        case enumPlayerPauseBegin:
            //played duration 
            if (_lastRunState==enumPlayerPlayBegin) {
                [self insertDurationStateLog:enumPlayerPlayDuration duration:stateDuration curTimestamp:curTimestampNumber pos:nil];
            }else if (_lastRunState==enumPlayerBeginLoadBegin) {
                [self insertDurationStateLog:enumPlayerBeginLoadDuration duration:stateDuration curTimestamp:curTimestampNumber pos:curPosition ];
            }else if(_lastRunState==enumPlayerSeekLoadBegin){
                [self insertDurationStateLog:enumPlayerSeekLoadDuration duration:stateDuration curTimestamp:curTimestampNumber pos:curPosition];
            }else if(_lastRunState==enumPlayerNormalLoadBegin){
            ...
            break;
        case enumPlayerSeekBegin:
            //played duration or pause duration
            if (_lastRunState==enumPlayerPlayBegin) {
                [self insertDurationStateLog:enumPlayerPlayDuration duration:stateDuration curTimestamp:curTimestampNumber pos:nil];
            }else if(_lastRunState==enumPlayerPauseBegin){
                [self insertDurationStateLog:enumPlayerPauseDuration duration:stateDuration curTimestamp:curTimestampNumber pos:nil];
        ...
        case enumPlayerExit:
            ...
            break;
        default:
            //other operaion state do nothing
            break;
    }
    
    //update state and timestamp
    [self updateStateTimestamp:state timestamp: curTimestampNumber pos:curPosition];
}
...
- (void)prepareToPlay: (NSTimeInterval) startTime
{
    ...
    _startTimePos = startTime*1000;
    //not work well
    ijkmp_set_option_int(_mediaPlayer, IJKMP_OPT_CATEGORY_PLAYER, "seek-at-start", _startTimePos);
    ...
    if ( _isReportLog) {
        [self reportCurrentState:enumPlayerBeginLoadBegin];
    }
...
- (void)play
{
    ...
    [self startHudTimer];
    if (_isReportLog) {
        [self reportCurrentState:enumPlayerPlayBegin];
    }
...
//    [self stopHudTimer];
    ...
    if ( _isReportLog) {
         [self reportCurrentState:enumPlayerPauseBegin];
    }
}
...
- (void)setIsReportLog:(BOOL) isReportLog
{
    if (!_mediaPlayer) {
        return;
    }
    _isReportLog = isReportLog;
    ijkmp_set_option_int(_mediaPlayer, IJKMP_OPT_CATEGORY_PLAYER, "post_start_log", isReportLog?1:0);
    //ijkmp_set_log_reportdb(_mediaPlayer, _isReportLog ? 1 : 0);
}
...
- (Boolean)setPreDNSInfo:(NSDictionary<NSString*, NSArray*>*) dicIPs{
    if ( dicIPs && dicIPs.count>0 ) {
        NSString* dnsInfo = @"";
        NSString* speratorDomain = @";";
        NSString* speratorIP = @",";
        NSArray* domainArray =  dicIPs.allKeys;
        for ( int i=0; i<dicIPs.count; i++) {
            NSString* domainName = domainArray[i];
            dnsInfo = [dnsInfo stringByAppendingString:domainName];
            NSMutableArray* arrayIPs =  (NSMutableArray*)[dicIPs objectForKey: domainArray[i]];
            for ( int j=0; j<arrayIPs.count; j++) {
                dnsInfo = [dnsInfo stringByAppendingString:speratorIP];
                dnsInfo = [dnsInfo stringByAppendingString:arrayIPs[j]];
            }
            dnsInfo = [dnsInfo stringByAppendingString:speratorDomain];
        }
        if ( dnsInfo.length>0) {
            [self setPlayerOptionValue: dnsInfo forKey:@"star_DNS_set"];
            NSLog(@"set pre dns info: %@", dnsInfo);
            return YES;
        }
...
- (NSTimeInterval)durationMilliSeconds
{
    if (!_mediaPlayer)
        return 0.0f;
...
- (void)shutdown
{
    ...
    if ( _isReportLog && _lastRunState!=enumPlayerExit ) {
        [self reportCurrentState:enumPlayerExit];
        [self insertCommonLogs];
        [[STLogReporter instance] postLog: _logDBUrl];
    }
    [_networkInfo uninitNetworkInfo];
    
    
    [self performSelectorOnMainThread:@selector(shutdownWaitStop:) withObject:self  waitUntilDone:YES];
    //[self performSelectorInBackground:@selector(shutdownWaitStop:) withObject:self];
}

-(void) insertCommonLogs{
    if (_networkInfo) {
        [[STLogReporter instance] insertCommLog:KEY_NETWORK value: [NSNumber numberWithInt:_networkInfo.networkType]];
        [[STLogReporter instance] insertCommLog:KEY_AREA value: _networkInfo.areaName];
        [[STLogReporter instance] insertCommLog:KEY_IPAREA value:_networkInfo.ipAreaName];
        [[STLogReporter instance] insertCommLog:KEY_OPERATOR value: _networkInfo.operatorName];
    }
    
    NSTimeInterval videoDuration = [self durationMilliSeconds];
    [[STLogReporter instance] insertCommLog:KEY_VIDEO_TOTALTIME value: [NSNumber numberWithLong:videoDuration]];
    [[STLogReporter instance] insertCommLog:KEY_EQUIPID value:[STDeviceID getDeviceID]];
    [[STLogReporter instance] insertCommLog:KEY_PLAY_URL value:_urlString];
    [[STLogReporter instance] insertCommLog:KEY_LIVE value:[NSNumber numberWithInt:_isLive ]];
    NSString* version = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleShortVersionString"];
    [[STLogReporter instance] insertCommLog:KEY_APP_VERSION value: version];
...
- (int)selectStream: (int) stream flag:(int) onoff{
    if (!_mediaPlayer)
        return -1;
    int selected = onoff==0? 0: 1;
    return ijkmp_set_stream_selected(_mediaPlayer, stream, selected);
}


-(int)selectedSubtitle{
   return (int)ijkmp_get_property_int64(_mediaPlayer, FFP_PROP_INT64_SELECTED_SUBTITLE_STREAM, -1 );
}

-(int)selectedAudio{
    return (int)ijkmp_get_property_int64(_mediaPlayer, FFP_PROP_INT64_SELECTED_AUDIO_STREAM, -1 );
}

- (NSDictionary*)getAudioTracks{
    @try{
        if (_monitor.mediaMeta!=nil) {
            NSMutableDictionary* dic = [[NSMutableDictionary alloc] init];
            NSMutableArray* array =  _monitor.mediaMeta[kk_IJKM_KEY_STREAMS];
            if (!array) {
                return nil;
            }
            ...
            for (int i=0; i<array.count; i++) {
                NSMutableDictionary* dicMeta = array[i];
                NSString* type = [dicMeta objectForKey:@IJKM_KEY_TYPE];
                if ([type isEqual:@IJKM_VAL_TYPE__AUDIO] ) {
                    
                    NSString* trackName = dicMeta[@IJKM_KEY_LANGUAGE];
                    if (trackName==nil) {
                        trackName = @"Track_";
                        trackName = [trackName stringByAppendingFormat:@"%d", index];
                    }
                    [dic setObject: trackName forKey:[NSString stringWithFormat:@"%d", i]];
                    index++;
                }
            }
            return dic;
        ...
        NSLog(@"getAudioTracks exception=%@", exception);
    }
    return nil;
    ...
    @try{
        if (_monitor.mediaMeta!=nil) {
            ...
            for (int i=0; i<array.count; i++) {
                NSMutableDictionary* dicMeta = array[i];
                NSString* type = [dicMeta objectForKey:@IJKM_KEY_TYPE];
                if ([type isEqual:@IJKM_VAL_TYPE__SUBTITLE] ) {
                    NSString* trackName = dicMeta[@IJKM_KEY_LANGUAGE];
                    if (trackName==nil) {
                        trackName = @"Track_";
                        trackName = [trackName stringByAppendingFormat:@"%d", index];
        ...
        NSLog(@"getSubtitleTracks exception=%@", exception);
    }
    return nil;
}
...
- (void)seek:(NSTimeInterval)aCurrentPlaybackTime
{
    if (!_mediaPlayer)
        return;
    
    //Fix bug of seek to end
    _seekTargetTime = aCurrentPlaybackTime;
    if (aCurrentPlaybackTime>=duration) {
        _seekTargetTime -= 1.0;
    }
    ...
    if (_isReportLog) {
        [self reportCurrentState:enumPlayerSeekBegin];
    }
    
    _bufferingPosition = 0;
    ijkmp_seek_to(_mediaPlayer, _seekTargetTime * 1000);
}
...
-(NSTimeInterval) currentPlaybackTimeMilliSeconds{
    if (!_mediaPlayer)
        return 0.0f;
...
//                  forKey:@"tcp-spd"];
    ...
    [_glView setHudValue:formatedDurationMilli(_monitor.lastPrerollDuration) forKey:@"t-buffer"];
...
//             object:self];
        ...
        case FFP_MSG_PREPARED: {
            ...
            if (rawMeta) {
                ...
                fillMetaInternal(newMediaMeta, rawMeta, IJKM_KEY_SUBTITLE_STREAM, nil);
                ...
                for(size_t i = 0; i < count; ++i) {
                    ...
                    if (streamRawMeta) {
                        ...
                        if (type) {
                            ...
                            } else if(0== strcmp(type, IJKM_VAL_TYPE__SUBTITLE)){
                                fillMetaInternal(streamMeta, streamRawMeta, IJKM_KEY_LANGUAGE, nil);
                            }
                        }
                    }
        ...
        case FFP_MSG_COMPLETED: {
            ...
            [[NSNotificationCenter defaultCenter]
             postNotificationName:IJKMPMoviePlayerPlaybackDidFinishNotification
             object:self
             userInfo:@{IJKMPMoviePlayerPlaybackDidFinishReasonUserInfoKey: @(avmsg->arg1==0?IJKMPMovieFinishReasonPlaybackEnded:IJKMPMovieFinishReasonPlaybackSeekEnded)}];
...
//             object:self];
        ...
        case FFP_MSG_BUFFERING_START: {
            ...
            if (_isReportLog) {
                if (_lastRunState==enumPlayerSeekBegin) {
                    [self reportCurrentState:enumPlayerSeekLoadBegin];
                }else if (_lastRunState==enumPlayerBeginLoadBegin ) {
                    //[self reportCurrentState:enumPlayerBeginLoadBegin];
                }else{
                    [self reportCurrentState:enumPlayerNormalLoadBegin];
                }
        ...
        case FFP_MSG_BUFFERING_END: {
            ...
            int state = [self playbackState];
            NSLog(@"FFP_MSG_BUFFERING_END: state=%d\n", state);
            if (_isReportLog) {
                if ( state==IJKMPMoviePlaybackStatePaused) {
                    //缓冲之后继续暂停
                    [self reportCurrentState:enumPlayerPauseBegin];
                }else{
                    //缓冲之后播放
        ...
        case FFP_MSG_PLAYBACK_STATE_CHANGED:
            {
                //chenwq add for seek_at_start
//                if (_mediaPlayer&&ijkmp_is_playing(_mediaPlayer) && _startTimePos!=0 ) {
//                    NSLog(@"seek at start, pos=%d", _startTimePos);
//                    if(0== ijkmp_seek_to(_mediaPlayer, _startTimePos )){
//                        _startTimePos = 0;
//                    }
//                }
        ...
        case FFP_MSG_SEND_START_LOG:{
            [IJKMediaUtils printLog:[NSString stringWithUTF8String:(char*)avmsg->arg1]];
            if (_isReportLog) {
                NSString* logmsg = [NSString stringWithCString:(char*)avmsg->arg1 encoding: NSASCIIStringEncoding];
                free((void*)avmsg->arg1);
                NSArray* arrayParam = [logmsg componentsSeparatedByString:@":"];
                if (arrayParam.count>3) {
                    int logtype = [[arrayParam objectAtIndex:1] intValue];
                    if ( logtype==STAR_TIME_LOG_MAIN || logtype==STAR_TIME_LOG_TCP) {
                        NSString* logvalue = [arrayParam objectAtIndex:arrayParam.count-1];
                        NSArray* keyvalue = [logvalue componentsSeparatedByString:@"="];
                        if (keyvalue.count==2) {
                            long long value = [[keyvalue objectAtIndex:1] longLongValue];
                            [[STLogReporter instance] insertPlayLog:[keyvalue objectAtIndex:0] value: [NSNumber numberWithLongLong: value] ];
                        }
                    }
                }
